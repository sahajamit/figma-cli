...
function formatHuman(response: FigmaStylesResponse): string {
  ...
  const lines: string[] = [];
  lines.push(chalk.bold(`Found ${styles.length} published style(s)`));
  lines.push('');

  if (styles.length === 0) return lines.join('\n');

  const headers = ['Name', 'Type', 'Key', 'Description'];
...
export function registerTeamCommand(styles: Command): void {
  styles
    .command('team <team-id>')
    .description('List published team styles')
    .action(async (teamId: string, _opts: unknown, command: Command) => {
      ...
      const result = await client.getTeamStyles(teamId);
